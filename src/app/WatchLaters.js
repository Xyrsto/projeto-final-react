...
class WatchLaters extends Component 
{
    ...
    {
        super(props);
        this.state = {loggedUser: "", listaWL: [], listaConteudos: [], htmlCont: []}

    }

    async componentDidMount(){
        await this.getUser();
        var url = `/api/conteudosAPI/getWatchLaters/${this.state.loggedUser}`
    ...
    async buscarFavs(url) {
        ...
        .then(res => res.json())
        .then(result => {
            this.state.listaWL.push(result.value)
            this.getFilmes()
        })
        .catch(error => console.log('error', error));
    }
    ...
    async getFilmes(){
        var requestOptions = {
            ...
            this.state.listaWL[0].forEach(async contId => {
